...
const drop = () => {
    // drag * спрацюють лише, якшо перетягувати елементи безпосередньо на сторінці, не з папки комп"ютера
    // dragend *
    ...
    ['dragleave', 'drop'].forEach(eventName => {
        fileInputs.forEach(input => {
            input.addEventListener(eventName, () => removeHighlight(input), false);
        })
    });    
